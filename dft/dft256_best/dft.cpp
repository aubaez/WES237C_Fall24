...
void dft(DTYPE real_sample[SIZE], DTYPE imag_sample[SIZE], DTYPE Out_R[SIZE], DTYPE Out_I[SIZE])
{
	...
	for(int i = 0; i < SIZE; i++){
		...
		for(int j = 0; j < SIZE; j++){
			//#pragma HLS ARRAY_PARTITION variable= cos_coefficients_table type=block factor=2 dim=2
			//#pragma HLS ARRAY_PARTITION variable= sin_coefficients_table type=block factor=2 dim=2
